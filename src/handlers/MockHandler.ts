...
import { IModelHandler } from "../shared"
import {
  IFields,
  IFilters,
  IItemDetails,
...
export class MockHandler implements IModelHandler {
  ...
  search(
    ...
    const records =
      typeof page.offset !== "undefined" && typeof page.limit !== "undefined"
  ...
  listRelationships(
    ...
    if (Array.isArray(rel.data)) {
      ...
      return Promise.resolve({
        item:
          relationshipRecord &&
          this.serializers[d.type].serialize(relationshipRecord),
        includes:
          relationshipRecord &&
          this.loadIncludes([relationshipRecord], include, fields),
      })
    }
  }
  ...
  addRelationships(
    ...
    const currentData = serializedRecord.relationships[relationship]
      .data as Array<IResourceIdentifier>
    serializedRecord.relationships[relationship].data = currentData.concat(
      items,
    )
  ...
  update(
    ...
    const serializedUpdate = cloneDeep(data)
    const serializedUpdateRelationships = serializedResource.relationships

    const mergedRelationships = merge(
      serializedResourceRelationships,
      serializedUpdateRelationships,
    )
  ...
  destroyRelationships(
    ...
    const relData = serializedRecord.relationships[relationship].data as Array<
      IResourceIdentifier
    >
    serializedRecord.relationships[relationship].data = relData.filter(
  ...
  private filterItems(
    items: Array<IResourceComplete>,
    filters: IFilters,
  ): Array<IResourceComplete> {
