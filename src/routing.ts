...
import { IListDetails, IPagingParams } from "./types"
...
export function handleGet(
  ...
  return async (req, res, next) => {
    const {
      query: { page, include, filter, sort, fields },
    } = req

    const paging = {
      ...defaultPaging,
      ...(page && (page as any).limit
        ? { limit: parseInt((page as any).limit, 10) }
        : {}),
      ...(page && (page as any).offset
        ? { offset: parseInt((page as any).offset, 10) }
        : {}),
    } as IPagingParams
    ...
    const includes = include
      ? Array.isArray(include)
        ? include
        : (include as string).split(",")
      : []

    const filters = destringifyBooleans(filter || {})

    const sortData = sort ? (sort as string).split(",") : null

    const fieldData = Object.keys(fields || {}).reduce((sum, modelName) => {
      sum[modelName] = sum[modelName]
        ? Array.isArray(sum[modelName])
          ? sum[modelName]
          : ((sum[modelName] as unknown) as string).split(",")
        : []
...
function generatePagingInformation(
  listData: any,
  apiPrefix: string,
  cleanedQuery: any,
  paging: any,
  ...
  if (paging.limit !== 0) {
    ...
    const lastPageOffset =
      Math.floor(listData.totalItems / paging.limit) * paging.limit
    lastQuery.page.offset = lastPageOffset
    lastPath =
      paging.limit < 0 ? firstPath : qs.stringify(lastQuery, { encode: false })

    const nextQuery = cloneDeep(cleanedQuery)
    nextQuery.page = firstQuery.page || {}
    nextQuery.page.offset = (paging.offset as number) + (paging.limit as number)
    nextPath =
      paging.limit < 0 || nextQuery.page.offset > lastPageOffset
        ? null
    ...
    prevPath =
      paging.limit < 0 || prevQuery.page.offset < 0
        ? null
...
function handleDelete(
  handler: IDeleteHandlerFunction,
  successStatus: number,
): express.RequestHandler {
  return async (req, res, next) => {
    try {
...
export function createModelRouter(
  ...
  router.post(
    "/",
    handlePost(
      ({ body: { data } }) => handler.create(type, data),
      201,
      renderItemResult,
    ),
  )
  ...
  router.post(
    "/:id(\\d+)/relationships/:relationship",
    handlePost(
      ({ body: { data }, params: { id, relationship } }) =>
  ...
  router.patch(
    "/:id(\\d+)",
    handlePatch(
      ({ body: { data }, params: { id } }) => handler.update(type, id, data),
      200,
      renderItemResult,
    ),
  )
  ...
  router.delete(
    "/:id(\\d+)",
    handleDelete(({ params: { id } }) => handler.destroy(type, id), 204),
  )
